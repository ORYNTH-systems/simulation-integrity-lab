...
from goals import AdmissibleStateGoal
from optimizer import ConstitutionalOptimizer, ShortestPathStrategy
...
goal = AdmissibleStateGoal()

candidate_nodes = GoalEvaluator().satisfying_nodes(
    graph,
    goal
)

result = ConstitutionalOptimizer().optimize(
    graph,
    candidate_nodes,
...
print(json.dumps({
    "optimization": result,
    "status": "CONSTITUTIONAL_OPTIMIZATION_ACTIVE"
}, indent=4))
